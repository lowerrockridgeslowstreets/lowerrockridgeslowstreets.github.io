...
Secrets (not in git): ~/.config/rockridge-formspree.env, or FORMSPREE_ENV_FILE / environment variables.
...
CONFIG_ENV_PATH = Path.home() / ".config" / "rockridge-formspree.env"
...
def main() -> int:
    ...
    if not api_key:
        print(
            "Missing FORMSPREE_API_KEY. Put FORMSPREE_API_KEY (and optional "
            "FORMSPREE_FORM_HASHID) in "
            f"{CONFIG_ENV_PATH}, set FORMSPREE_ENV_FILE, or export in the environment.",
            file=sys.stderr,
        )
        return 1
