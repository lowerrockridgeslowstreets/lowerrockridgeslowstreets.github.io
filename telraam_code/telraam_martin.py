"""
Telraam data puller — Jeremiah's counter, Martin St, Oakland
Segment ID: 9000010647

Pulls hourly traffic via the Telraam API, aggregates to calendar days (segment
timezone), and writes a CSV with site labels.

Usage:
    export TELRAAM_API_KEY='…'  (optional if ~/.config/telraam/telraam exists)
    ~/pipx/shared/bin/python3 telraam_martin.py

Output (under ``<DATA_DIR>/API_Data/``; see ``telraam_paths`` / ``TELRAAM_DATA_DIR``):
    telraam_martin_data.csv
    telraam_martin_hourly.csv
"""
...
SENSOR_ID = "9000010647"
API_URL = "https://telraam-api.net/v1/reports/traffic"
# Installation from Telraam location page: 25/02/2026 (S2, Counting)
...
OUTPUT_FILE = API_DATA_DIR / "telraam_martin_data.csv"
OUTPUT_HOURLY = API_DATA_DIR / "telraam_martin_hourly.csv"
INSTALLATION_ID = 9000010647
STREET_NAME = "Martin St"
CITY_NAME = "Oakland"
LABEL = "Jeremiah's"
LABEL_NOTE = "Martin St (yellow marker)"
...
def main():
    ...
    print(f"Telraam export — {LABEL} / Martin St (segment {SENSOR_ID})")
